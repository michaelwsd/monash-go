...
def wired(
...
) -> Iterator[tuple[TestClient, FakeRideRepo, FakeRouteRepo, FakeBookingRepo]]:
    ...
    rides, routes, bookings = FakeRideRepo(), FakeRouteRepo(), FakeBookingRepo()
    ...
    monkeypatch.setattr(ride_service, "booking_repository", bookings)
    ...
    yield TestClient(app), rides, routes, bookings
...
#
# CLAUDE.md: "Phone numbers only revealed after booking is confirmed." These
# assert the key is genuinely ABSENT, not present-and-null: a null still tells
# the frontend the field exists, and a schema that declares it is one edit away
# from filling it in.


def test_a_stranger_never_sees_the_drivers_phone(
    wired: tuple[TestClient, FakeRideRepo, FakeRouteRepo, FakeBookingRepo],
    make_token: Callable[..., str],
) -> None:
    client, _, _, bookings = wired
    bookings.has_confirmed_seat = False
    token = make_token(sub=CLERK_ID)
    created = post_a_ride(client, token)

    driver = detail_of(client, token, created["id"]).json()["driver"]

    assert "phone" not in driver
    assert OWNER.phone not in str(driver)


def test_a_confirmed_passenger_sees_the_phone(
    wired: tuple[TestClient, FakeRideRepo, FakeRouteRepo, FakeBookingRepo],
    make_token: Callable[..., str],
) -> None:
    """The other half. Without this, a response that always omitted the number
    would pass the test above and the feature would never work."""
    client, _, _, bookings = wired
    token = make_token(sub=CLERK_ID)
    created = post_a_ride(client, token)
    ...
    assert driver["phone"] == OWNER.phone


def test_a_cancelled_booking_takes_the_phone_back(
    wired: tuple[TestClient, FakeRideRepo, FakeRouteRepo, FakeBookingRepo],
    make_token: Callable[..., str],
) -> None:
    ...
    bookings.has_confirmed_seat = True
    assert "phone" in detail_of(client, token, created["id"]).json()["driver"]

    bookings.has_confirmed_seat = False
    assert "phone" not in detail_of(client, token, created["id"]).json()["driver"]


def test_the_rest_of_the_response_is_the_same_either_way(
    wired: tuple[TestClient, FakeRideRepo, FakeRouteRepo, FakeBookingRepo],
    make_token: Callable[..., str],
) -> None:
    """RideDetailWithContact narrows one field of RideDetail and inherits the
    rest, so the two must not drift. Only `driver` may differ."""
    client, _, _, bookings = wired
    token = make_token(sub=CLERK_ID)
    created = post_a_ride(client, token)

    without = detail_of(client, token, created["id"]).json()
    bookings.has_confirmed_seat = True
    with_phone = detail_of(client, token, created["id"]).json()
